...
<div class="container mx-auto px-4 py-8" x-data="{ 
    ...
    <!-- Header Wadah -->
    <div class="bg-gradient-to-r from-teal-500 to-teal-600 p-4 {{ count($students) > 0 ? 'rounded-t-xl rounded-b-none border-b border-teal-600/50' : 'rounded-xl mb-6 shadow-sm' }} flex flex-col md:flex-row justify-between items-center text-white gap-4 transition-all">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto justify-end">
            ...
            <div x-data="{ openDropdown: false, selectedSchoolName: '{{ $schools->where('id', request('school_id'))->first()->name ?? 'Semua Sekolah' }}' }" class="relative">
                <button type="button" @click="openDropdown = !openDropdown" @click.away="openDropdown = false" class="bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg px-4 py-2 text-sm font-medium flex items-center justify-between gap-2 transition-colors w-full">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4" x-ref="grid">
            @forelse($students as $student)
                <div x-show="search === '' || $el.innerText.toLowerCase().includes(search.toLowerCase())"
                     ...
                     class="student-card-data bg-white rounded-xl border border-gray-100 p-5 relative flex flex-col justify-between h-full">
                    <div>
