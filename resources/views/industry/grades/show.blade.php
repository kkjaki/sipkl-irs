...
<div class="container mx-auto px-4 py-8" x-data="{ 
    ...
    <!-- Header Wadah -->
    <div class="bg-gradient-to-r from-teal-500 to-teal-600 p-4 {{ count($students) > 0 ? 'rounded-t-xl rounded-b-none border-b border-teal-600/50' : 'rounded-xl mb-6 shadow-sm' }} text-white flex flex-col sm:flex-row justify-between items-center gap-4 transition-all">
    ...
    <div class="bg-white rounded-b-xl border border-gray-100 shadow-sm p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" x-ref="grid">
            ...
            <div x-show="search === '' || $el.innerText.toLowerCase().includes(search.toLowerCase())"
                 ...
                 class="student-card-data bg-white rounded-xl border border-gray-100 p-6 relative flex flex-col justify-between h-full">
                <div>
    ...
    <!-- Navigation Kembali -->
    <a href="{{ route('grades.schools.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-all duration-200 text-sm mt-8">
