...
<div class="container mx-auto px-4 py-8">
    <!-- Header Wadah -->
    <div class="bg-brand-primary p-4 rounded-t-xl shadow-sm text-white mb-6 flex justify-between items-center">
        <h2 class="text-xl font-bold m-0">Tambah Penilaian Siswa</h2>
        <a href="{{ route('grades.schools.show', $school->id) }}" class="text-white hover:text-teal-200 text-sm font-medium transition-colors">
            &larr; Kembali
        </a>
    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <form method="POST" action="{{ route('grades.schools.update', [$school->id, $student->id]) }}">
                            ...
                            <tbody>
                                @foreach($grades as $index => $grade)
                                    @php
                                        ...
                                        $critName = $criterion->firstWhere('id', $grade->criteria_id)->name ?? 'Kriteria tidak diketahui';
                                    @endphp
                                    <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="p-4">
                                            ...
                                            <input type="number" name="grades[{{ $index }}][score]" value="{{ $grade->score }}" class="score-input w-full border border-gray-300 rounded-lg p-2 text-center focus:ring-brand-primary focus:border-brand-primary transition-colors" min="0" max="100" required>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div class="p-6 bg-white border-t border-gray-100 flex justify-end gap-3">
                        <a href="{{ route('grades.schools.show', $school->id) }}" class="px-5 py-2.5 rounded-lg font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors">
                            Batal
                        </a>
                        <button type="submit" class="px-5 py-2.5 rounded-lg font-medium text-white bg-brand-primary hover:bg-teal-700 transition-colors shadow-sm">
                            Simpan Penilaian
                        </button>
                    </div>
                </form>
