@extends('layouts.app')
@section('title', 'Validasi Presensi Siswa')
...
@section('content')
    <main class="min-h-screen bg-brand-bg px-10 pb-10 w-full" x-data="{
        ...
        <section class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden mb-8">
            ...
            <div class="p-6">
                <div class="max-w-md">
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Nama Sekolah /
                        Instansi</label>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @forelse($students as $student)
                        ...
                        <section
                            class="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 p-6 relative group"
                            ...
                            <div class="absolute top-6 right-6">
                                <button @click="showModal = true" class="focus:outline-none group/badge">
                                    ...
                                    @endphp
                                    <span
                                        class="px-2 py-1 text-[10px] font-black uppercase tracking-widest {{ $textColor }} flex items-center gap-1 group-hover/badge:bg-gray-100 rounded-lg transition-all">
                                        {{ $statusDisplay }}
                            ...
                            <div class="pr-12">
                                ...
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1 mb-5">NIS:
                                    {{ $student->nis ?? '-' }}</p>

                                <div
                                    class="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 group-hover:bg-white transition-colors relative overflow-hidden">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="text-xs font-bold text-gray-600">
                                            @if ($attendance && $attendance->check_in)
                                                Check In: <span
                                                    class="text-gray-900">{{ \Carbon\Carbon::parse($attendance->check_in)->format('H:i') }}
                                                    WIB</span>
                                            @else
                            ...
                            <div class="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                                ...
                                @else
                                    <span
                                        class="flex items-center gap-2 text-[10px] font-bold text-gray-300 px-4 py-2 border border-dashed border-gray-200 rounded-lg">
                            ...
                            <div x-cloak x-show="showModal" class="fixed inset-0 z-[9999] overflow-y-auto">
                                <div class="flex items-center justify-center min-h-screen p-4">
                                    ...
                                    <div class="bg-white rounded-2xl p-6 w-full max-w-md relative z-[10000] shadow-2xl"
                                        ...
                                        <div class="mb-8">
                                            <label
                                                class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Status
                                                Baru</label>
                                            <select x-model="statusUpdate"
                                                class="w-full rounded-xl border-gray-200 bg-gray-50 py-3 px-4 font-bold text-gray-800 focus:ring-teal-500 transition-all">
                                        ...
                                        <div class="flex gap-3">
                                            ...
                                            <button
                                                @click="updateAttendance({{ $student->id }}, {{ $sessionId }}, statusUpdate); showModal = false"
                    ...
                    @empty
                        <div
                            class="col-span-full py-20 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-300">
        ...
        {{-- SUCCESS ALERT MODAL --}}
        <div x-cloak x-show="successAlert" class="fixed inset-0 z-[10001] flex items-center justify-center px-4"
            style="display: none;">
            <div x-show="successAlert" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" class="absolute inset-0 bg-black/60 backdrop-blur-sm"
                @click="successAlert = false; location.reload();"></div>
            ...
            <div x-show="successAlert" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <p class="text-sm text-gray-500 font-medium leading-relaxed mb-8">Data kehadiran siswa telah diperbarui dan
                    disimpan ke sistem.</p>
                <button @click="successAlert = false; location.reload();"
                    class="w-full py-4 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-lg transition-all active:scale-95 focus:outline-none">Tutup</button>
            </div>
        </div>
    </main>
