...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6" x-data="{
            ...
            <!-- Header Wadah -->
            <div
                class="bg-gradient-to-r from-teal-500 to-teal-600 text-white p-4 flex justify-between items-center relative rounded-t-xl">
                <h2 class="text-lg font-semibold m-0">Daftar Riwayat Kehadiran Siswa</h2>

                <!-- Tombol Filter -->
                <button @click="showFilterModal = true"
                    class="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-sm font-medium transition-colors">
                ...
                <div x-show="showFilterModal" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button @click="tempFilter = 'hadir'"
                                :class="tempFilter === 'hadir' ? 'bg-gray-800 text-white' :
                                    'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                                class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors">
                                Hadir
                            </button>
                            <button @click="tempFilter = 'izin'"
                                :class="tempFilter === 'izin' ? 'bg-gray-800 text-white' :
                                    'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                                class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors">
                                Izin
                            </button>
                            <button @click="tempFilter = 'sakit'"
                                :class="tempFilter === 'sakit' ? 'bg-gray-800 text-white' :
                                    'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                                class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors">
                                Sakit
                            </button>
                            <button @click="tempFilter = 'alpa'"
                                :class="tempFilter === 'alpa' ? 'bg-gray-800 text-white' :
                                    'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                                class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors">
                                Alpa
                            </button>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                    @forelse($students as $student)
                        ...
                        @endphp
                        <div class="bg-white rounded-xl border border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200 p-5 relative"
                            ...
                            <button @click="showModal = true" class="absolute top-4 right-4 focus:outline-none">
                                ...
                                @endphp
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-semibold border {{ $badgeClass }} transition-colors hover:opacity-80">
                                    {{ $statusDisplay }}
                                    <svg class="w-4 h-4 ml-1.5 inline-block" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="pr-16">
                                <h3 class="text-xl font-bold text-gray-800 mb-2">
                                    {{ $student->user->name ?? 'Nama Siswa Tidak Ditemukan' }}</h3>

                                <div class="text-sm text-gray-600 space-y-1 mb-4">
                                    <p>NIS: <span class="font-medium">{{ $student->nis ?? '-' }}</span></p>
                                    @if ($attendance && $attendance->check_in)
                                        <p>Waktu Check In: <span
                                                class="font-medium">{{ \Carbon\Carbon::parse($attendance->check_in)->format('H:i') }}
                                                WIB</span></p>
                                    @else
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center text-sm text-gray-400 cursor-not-allowed font-medium"
                                        title="Bukti Presensi Tidak Tersedia">
                            ...
                            <div x-show="showModal"
                                ...
                                <div x-show="showModal" x-transition:enter="transition ease-out duration-300 transform"
                                    ...
                                    <p class="text-sm text-gray-500 mb-4">Siswa: <span
                                            class="font-semibold">{{ $student->user->name ?? '-' }}</span></p>

                                    <div class="mb-5">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Status
                                            Kehadiran</label>
                                        <select x-model="statusUpdate"
                                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary focus:ring-opacity-50">
                                    ...
                                    <div class="flex justify-end gap-3 mt-6">
                                        ...
                                        <button type="button"
                                            @click="updateAttendance({{ $student->id }}, {{ $sessionId }}, statusUpdate)"
                    ...
                    @empty
                        <div
                            class="col-span-full flex flex-col items-center justify-center p-8 bg-gray-50 rounded-xl text-center">
