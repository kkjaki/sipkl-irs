...
@section('content')
    <main class="min-h-screen bg-brand-bg px-10 pb-10 w-full" x-data="{
        ...
        <article class="w-full bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
            {{-- Header Card Teal dengan Dropdown Ala Recap --}}
            <div
                class="bg-gradient-to-r from-teal-500 to-teal-600 px-6 py-3 flex flex-col md:flex-row justify-between items-center text-white gap-4">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    {{-- Dropdown Filter Sekolah (UI Seragam dengan Recap) --}}
                    <div x-data="{ openDropdown: false, selectedSchoolName: '{{ $schools->where('id', request('school_id'))->first()->name ?? 'Semua Sekolah' }}' }" class="relative w-full sm:w-auto">
                        ...
                        <div x-show="openDropdown" x-cloak x-transition:enter="transition ease-out duration-100"
                            ...
                            {{-- Route diarahkan ke index logbook --}}
                            ...
                            @foreach ($schools as $school)
                                <a href="{{ route('industry.logbooks.index', ['school_id' => $school->id]) }}"
            ...
            <div class="p-6">
                @if (count($logbooks) > 0)
                    {{-- Bulk Action --}}
                    <div
                        class="mb-8 flex flex-col sm:flex-row items-center justify-between bg-gray-50 p-5 rounded-2xl border border-gray-200 shadow-inner gap-4">
                        <div class="flex items-center gap-4 text-gray-700">
                            <input type="checkbox" id="selectAll"
                                class="rounded-md border-gray-300 text-teal-600 focus:ring-teal-500 w-6 h-6 cursor-pointer">
                            <div class="flex flex-col">
                                <label for="selectAll" class="text-sm font-black cursor-pointer">Pilih Semua</label>
                                <span id="selectedCount" class="text-[10px] font-bold text-teal-600 uppercase hidden">0
                                    Terpilih</span>
                            </div>
                        </div>
                        <div class="flex gap-3 w-full sm:w-auto">
                            <button @click="bulkTarget = 'approved'; showConfirmBulk = true" id="btnApproveBulk" disabled
                                class="flex-1 sm:flex-none bg-white text-green-600 border border-green-200 px-5 py-2.5 rounded-xl text-sm font-bold transition-all disabled:opacity-50 hover:bg-green-600 hover:text-white shadow-sm active:scale-95">Setujui</button>
                            <button @click="bulkTarget = 'rejected'; showConfirmBulk = true" id="btnRejectBulk" disabled
                                class="flex-1 sm:flex-none bg-white text-rose-600 border border-rose-200 px-5 py-2.5 rounded-xl text-sm font-bold transition-all disabled:opacity-50 hover:bg-rose-600 hover:text-white shadow-sm active:scale-95">Tolak</button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" x-ref="grid">
                        @foreach ($logbooks as $logbook)
                            <section x-show="search === '' || $el.innerText.toLowerCase().includes(search.toLowerCase())"
                                x-transition:enter="transition ease-out duration-200" x-data="{ showModal: false, statusUpdate: '{{ $logbook->status }}' }"
                                ...
                                <div class="absolute top-6 left-6 z-10">
                                    <input type="checkbox" value="{{ $logbook->id }}"
                                        class="logbook-checkbox rounded-md border-gray-300 text-teal-600 focus:ring-teal-500 w-5 h-5 cursor-pointer shadow-sm">
                                </div>
                                ...
                                <div class="absolute top-6 right-6">
                                    <button @click="showModal = true" class="focus:outline-none">
                                        ...
                                        @endphp
                                        <span
                                            class="px-2 py-1 text-[10px] font-black uppercase tracking-widest {{ $textColor }} flex items-center gap-1 hover:bg-gray-50 rounded-lg transition-all">
                                ...
                                <div class="mt-10">
                                    <h3
                                        class="text-xl font-black text-gray-900 group-hover:text-teal-600 transition-colors">
                                        {{ $logbook->student->user->name ?? '-' }}</h3>
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1 mb-5">
                                        {{ $logbook->student->school->name ?? '-' }}</p>
                                    <div
                                        class="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 group-hover:bg-white transition-colors">
                                ...
                                <div class="mt-auto pt-5 border-t border-gray-100 flex items-center justify-between">
                                    <div class="flex items-center gap-2 text-xs font-bold">
                                        @if ($logbook->documentation_file)
                                            <a href="{{ route('industry.logbooks.download', $logbook->id) }}"
                                                target="_blank"
                                                class="text-blue-600 bg-blue-50 px-4 py-2 rounded-lg border border-blue-100 hover:bg-blue-600 hover:text-white transition-all">Dokumen</a>
                                        @endif
                                        <a href="{{ route('industry.logbooks.edit', $logbook->id) }}"
                                            class="text-teal-700 bg-teal-50 px-4 py-2 rounded-lg border border-teal-100 hover:bg-teal-600 hover:text-white transition-all">Feedback</a>
                                    </div>
                                    <span
                                        class="text-[10px] font-bold text-gray-400 italic">{{ $logbook->created_at->translatedFormat('F d Y') }}</span>
                                </div>
                                        ...
                                        <div class="bg-white rounded-2xl p-6 w-full max-w-md relative z-[10000] shadow-2xl">
                                            ...
                                            <div class="mb-8 text-left">
                                                <select x-model="statusUpdate"
                                                    class="w-full rounded-xl border-gray-200 bg-gray-50 py-3 px-4 font-bold text-gray-800 focus:ring-teal-500 focus:border-teal-500 transition-all">
                                            ...
                                            <div class="flex gap-3">
                                                <button @click="showModal = false"
                                                    class="flex-1 px-4 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-all">Batal</button>
                                                <button
                                                    @click="updateLogbook({{ $logbook->id }}, statusUpdate); showModal = false"
                                                    class="flex-1 px-4 py-3 bg-teal-600 text-white font-bold rounded-xl shadow-lg hover:bg-teal-700 active:scale-95 transition-all">Simpan</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div x-show="!hasVisible" x-cloak
                        ...
                        <p class="text-gray-500 font-medium">Logbook siswa dengan nama "<span x-text="search"
                ...
                @else
                    <div class="py-20 text-center bg-gray-50 rounded-xl border border-dashed border-gray-300">
                        ...
                        <h3 class="text-lg font-bold text-gray-800">Tidak ada logbook ditemukan</h3>
                        <p class="text-gray-500 text-sm">Coba pilih sekolah lain atau periksa kembali nanti.</p>
                    </div>
                @endif
            </div>
        </article>
        ...
        <div x-cloak x-show="showConfirmBulk" class="fixed inset-0 z-[10001] flex items-center justify-center px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center border border-gray-100"
                ...
                <p class="text-sm text-gray-500 font-medium leading-relaxed mb-8">
                    Apakah Anda yakin ingin memproses <span class="font-black text-teal-600"
                        id="bulkConfirmCount">0</span> logbook terpilih untuk
                    <span class="font-black" :class="bulkTarget === 'approved' ? 'text-green-600' : 'text-rose-600'"
                        x-text="bulkTarget === 'approved' ? 'DISETUJUI' : 'DITOLAK'"></span>?
                </p>
                <div class="flex gap-3">
                    <button @click="showConfirmBulk = false"
                        class="flex-1 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-all">Batal</button>
                    <button @click="submitBulk(bulkTarget)"
                        class="flex-1 py-3 bg-teal-600 text-white font-bold rounded-xl shadow-lg hover:bg-teal-700 active:scale-95 transition-all">Ya,
                        Proses Semua</button>
                </div>
            </div>
        </div>
        ...
        {{-- MODAL ALERT SUKSES --}}
        <div x-cloak x-show="successAlert" class="fixed inset-0 z-[10002] flex items-center justify-center px-4">
            <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" @click="successAlert = false; location.reload();">
            </div>
            <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center border border-gray-100">
                ...
                <p class="text-sm text-gray-500 font-medium leading-relaxed mb-8">Data validasi logbook telah berhasil
                    diperbarui!</p>
                <button @click="successAlert = false; location.reload();"
                    class="w-full py-4 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-lg transition-all active:scale-95">Mantap!</button>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        function submitBulk(status) {
            ...
            fetch('{{ route('industry.logbooks.bulkValidate') }}', {
                ...
                .catch(() => {
                    console.error('Gagal proses massal.');
                });
        }
