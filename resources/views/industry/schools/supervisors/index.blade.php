...
<main class="min-h-screen bg-slate-50 px-4 md:px-10 py-6">
    ...
    {{-- Data Section --}}
    <div>
        <div class="bg-gradient-to-r from-teal-500 to-teal-600 px-6 py-4 flex justify-between items-center rounded-t-xl text-white">
            <div class="flex items-center gap-2.5">
                <x-heroicon-o-user-group class="w-6 h-6"/>
                <h2 class="font-bold text-lg m-0">Data Guru Pendamping</h2>
            </div>
            <a href="{{ route('schools.supervisors.create', $school->id) }}" class="bg-white text-teal-600 hover:bg-gray-50 px-4 py-2 rounded-md text-sm font-semibold flex items-center gap-2 transition-colors shadow-sm">
        ...
        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 bg-white border border-t-0 border-gray-100 rounded-b-xl shadow-sm">
            ...
            @forelse ($supervisors as $supervisor)
                <div x-data="{ open: false }" class="bg-white rounded-xl border border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200 p-5 flex flex-col h-full">
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-lg font-bold text-gray-800" title="{{ $supervisor->name }}">{{ $supervisor->name }}</h3>
                        </div>
                        <span class="bg-teal-50 text-teal-600 px-3 py-1 rounded-full text-xs font-semibold border border-teal-100">Guru</span>
                    </div>
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-100 flex gap-2">
                        <a href="{{ route('supervisors.edit', $supervisor->id) }}" class="flex-1 flex justify-center items-center gap-2 bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200 py-2 px-3 rounded-lg text-sm font-medium transition-colors">
                            <x-heroicon-o-pencil class="w-4 h-4" /> Edit
                        </a>
                        
                        <button @click="open = true" class="flex-1 flex justify-center items-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 py-2 px-3 rounded-lg text-sm font-medium transition-colors">
            ...
            @empty
                <div class="col-span-full w-full text-center py-12 rounded-xl bg-gray-50 border border-dashed border-gray-200">
